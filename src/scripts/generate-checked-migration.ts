...
async function main(): Promise<void> {
  ...
  if (diff.created.length === 0 && diff.updated.length === 0 && diff.deleted.length === 0) {
    console.log('No hi ha canvis d esquema pendents de migracio.');
    return;
  }

  const issues = validateGeneratedMigrationArtifacts(diff);
  if (issues.length > 0) {
    await restoreDrizzleFiles(projectRoot, before);
    throw new Error(issues.join('\n'));
  }

  console.log(`Migracio generada correctament: ${diff.created.join(', ')}`);
}
...
async function runDrizzleGenerate(projectRoot: string): Promise<void> {
  await new Promise<void>((resolve, reject) => {
    const child = spawn('npm', ['exec', 'drizzle-kit', '--', 'generate', '--config', 'drizzle.config.ts'], {
      cwd: projectRoot,
      stdio: 'inherit',
      shell: false,
    });
