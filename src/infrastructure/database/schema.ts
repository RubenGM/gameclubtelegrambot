...
export const catalogItems = pgTable(
  'catalog_items',
  {
    ...
    storagePosition: varchar('storage_position', { length: 16 }),
  ...
  (table) => [
    check(
      'catalog_items_storage_position_format',
      sql`${table.storagePosition} is null or ${table.storagePosition} ~ '^[A-Z]{1,3}[1-9][0-9]{0,3}$'`,
    ),
  ],
);
...
export const scheduleEventParticipants = pgTable(
  ...
  (table) => ({
    ...
    companionCountNonNegative: check(
      'schedule_event_participants_companion_count_non_negative',
      sql`${table.companionCount} >= 0`,
    ),
    spectatorCountNonNegative: check(
      'schedule_event_participants_spectator_count_non_negative',
