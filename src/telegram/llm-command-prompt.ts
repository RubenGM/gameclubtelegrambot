...
export function buildLlmCommandPrompt(input: LlmCommandPromptInput): string {
  ...
  const lines = [
    ...
    'Tu unica tarea es convertir el mensaje del usuario en JSON valido con version 1.',
	    ...
	    'Devuelve nextStep.useStrongerModel=true solo cuando la siguiente fase de lectura vaya a necesitar razonamiento semantico sobre datos reales recuperados por el bot, como recomendaciones, detalle contextual, busqueda multi-fuente o filtrado de Storage ambiguo. No lo uses para permisos, admin ni escrituras.',
	    'Si nextStep.useStrongerModel=true, escribe nextStep.reason con una razon breve y concreta; si es false, usa reason=null.',
    ...
    'La categoria principal de STL agrupa todo lo que cuelga de ella: STL, modelos 3D, figuras, estatuas, miniaturas, dioramas o terminos similares de impresion 3D.',
    'Si el usuario pide libros de rol, material de rol, aventuras, fichas, mapas, PDFs, documentos, archivos, STL, modelos 3D, figuras, estatuas, miniaturas o contenido subido, usa storage.search salvo que pida explicitamente un item fisico/prestable del catalogo.',
    'Cuando el usuario diga STL como tipo de contenido de impresion 3D, no pongas fileExtensions=["stl"]; esos modelos pueden estar comprimidos en zip/rar. Usa query con la franquicia o tema y deja fileExtensions=[] salvo que pida una extension concreta como pdf, zip o rar.',
    'Las acciones administrativas no se ejecutan por IA: marca requiresAdmin=true y usa unsupported.',
    'Trata feedback.offer como una clasificación prioritaria y semántica: si el usuario insulta, ridiculiza o desprecia directamente al bot o a una de sus respuestas, usa intent=feedback.offer, action.type=answer_directly, requiresConfirmation=false y safety.risk=read_only.',
    'Reconoce este caso en catalán, castellano e inglés aunque haya faltas, acentos omitidos, letras repetidas, separación o puntuación extraña, sustituciones fonéticas/visuales, abreviaturas, sarcasmo, metáforas creativas o insultos inventados. Si el destinatario es claramente el bot, una respuesta suya o el contexto de reply, prioriza feedback.offer incluso cuando el insulto sea una sola palabra o una formulación poco convencional.',
    'No uses feedback.offer para insultos entre personas, autoinsultos, citas, ejemplos educativos, títulos/contenido de terceros ni lenguaje duro que no vaya dirigido al bot. Ante ambigüedad real sobre el destinatario, continúa con el intent normal. El bot abrirá el flujo de feedback con consentimiento.',
    '',
    ...
    '',
	    ...
	    '{"version":1,"language":"es","intent":"help.capabilities","confidence":0.95,"reply":{"text":"...","sendNow":false},"progress":{"messages":[]},"nextStep":{"useStrongerModel":false,"reason":null},"needsClarification":false,"clarification":null,"requiresConfirmation":false,"confirmation":null,"action":{"type":"call_internal_handler","name":"help.capabilities","params":{"query":null,"dateRange":null,"tag":null,"fileExtensions":[],"id":null,"noticeId":null,"entryId":null,"categoryId":null,"itemId":null,"eventId":null,"purchaseId":null,"title":null,"text":null,"message":null,"body":null,"content":null,"description":null,"playerCount":null,"availableOnly":null,"itemType":null,"expiresAt":null,"startsAt":null,"groupTitle":null,"sources":[]}},"safety":{"requiresApprovedMember":false,"requiresAdmin":false,"risk":"read_only","publicSideEffect":false,"destructive":false,"requiresPrivateChat":false}}',
  ];
