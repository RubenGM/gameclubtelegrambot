...
| Agenda de actividades                        | 🟢 Operativo        | Detalle con creador, asistentes y reservas asignables; promoción a destinos news, altas/bajas, conflictos y recordatorios.            |
...
- `/autojoin enabled` y `/autojoin disabled` permiten a admins activar por grupo el alta automatica: cada usuario no bot que entre en un grupo con autojoin activado se crea como pendiente si hace falta, queda aprobado como miembro y recibe siempre una bienvenida de grupo si hay plantillas activas, aunque ya constase como aprobado; con autojoin desactivado, la entrada al grupo no aprueba ni envia bienvenida. Los usuarios bloqueados no se reactivan automaticamente.
