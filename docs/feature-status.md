...
- Durante un flujo guiado ajeno a Storage, los nombres de categorías de archivos no capturan el texto libre: por ejemplo, `Anima` se conserva como sistema de juego aunque exista una carpeta de Storage con ese nombre. Los accesos explícitos a Storage siguen permitiendo cambiar de sección.
- `Editar partida` inicia un flujo guiado cancelable para cambiar título, sistema, descripción, plazas, visibilidad, modo de entrada, aceptación, programación manual por jugadores, día y hora preferidos, mesa predeterminada, publicación Agenda por defecto y estado. Sólo el GM principal/creador y los admins globales pueden modificar estos valores; los coorganizadores conservan únicamente la configuración operativa permitida.
...
- Las campañas activas muestran `Programar siguiente sesión` a GM/coorganizadores/admins y a jugadores confirmados cuando la partida permite programación manual por jugadores, incluso si tienen recurrencia configurada. Si la partida tiene día y hora preferidos, el bot calcula la siguiente fecha futura compatible y propone directamente `¿Quieres programar la siguiente partida el …, a las …?`, muestra la mesa predeterminada y ofrece `Aceptar`, `Modificar` y `Cancelar`: `Aceptar` crea la reserva en un clic después de revalidar permisos, valores y posible sesión sustituida; `Modificar` vuelve al recorrido manual sin escribir nada. Sin preferencias completas, o después de modificar, la acción crea o sustituye exclusivamente la próxima sesión: si ya existe, enseña su enlace y exige confirmar que se reemplaza; las sesiones posteriores se conservan. Tras introducir fecha y hora, el bot todavía no escribe nada: muestra nombre, día completo, franja horaria, duración, tipo, visibilidad, plazas, mesa y descripción, y exige pulsar `Confirmar`; `Cancelar` deja Agenda intacta.
- Cuando se elige publicar una campaña al crearla, la periodicidad pregunta explícitamente cada cuántas semanas se repetirá y ofrece `Sin días fijos`, `Cada semana` y `Cada 2 semanas` en el teclado; también acepta escribir un número de semanas entre 1 y 52. `Sin días fijos` vuelve al alta sin publicación y activa exclusivamente la programación manual; una frecuencia semanal pide el día de la semana mediante siete botones y después obliga a elegir la siguiente partida entre las cuatro próximas fechas coincidentes antes de pedir hora y ventana futura.
...
- `/schedule` con crear, crear en modo simple, listar, editar, cancelar, detalle por deep link, unirse y salir. El botón `Crear (simple)` pide título, fecha, hora, tipo de mesa y número de plazas, y crea directamente una actividad de 180 min sólo para socios, 0 ocupadas, sin mesa, sin equipamiento y sin descripción. El flujo completo muestra al elegir fecha la agenda de ese día con el mismo formato de publicación y, tras la hora, propone sin duración, mesa cerrada y sin mesa; el resumen identifica claramente `Tipo de mesa` para cambiarlo antes de guardar. La edición permite cambiar una actividad entre mesa abierta y cerrada, además de duración, plazas, mesa y equipamiento reservado.
...
- Asistencia con distinción clara entre jugadores, acompañantes a jugar y espectadores:
  - En actividades abiertas, un usuario puede apuntarse en un clic como Jugador (`🎮 Apuntarme a jugar`, consume 1 plaza) o como Espectador (`👀 Apuntarme de espectador`, consume 0 plazas).
  - Si una mesa se llena de jugadores, el botón principal pasa a ser `👀 Apuntarme de espectador (mesa llena)` permitiendo que sigan apuntándose espectadores sin consumir plazas de juego.
  - Plazas ocupadas y aforo: la ocupación computa de forma estricta las plazas iniciales ocupadas (`initialOccupiedSeats`) más la suma de cada jugador y sus acompañantes a jugar (`1 + companionCount`). Los espectadores (socio o adicionales) consumen 0 plazas.
  - Acompañantes a jugar (`companionCount`): disponibles exclusivamente para participantes con rol de jugador mediante el botón `👥 Acompañantes a jugar (+X)` con submenú interactivo (+1, -1, 0). Cada acompañante consume 1 plaza de la mesa en tiempo real; el incremento está sujeto a la disponibilidad de plazas libres en la mesa y rechaza adiciones si la mesa está completa. Si un jugador pasa a rol de espectador, sus acompañantes a jugar se resetean automáticamente a 0 liberando sus plazas de juego.
  - Espectadores adicionales (`spectatorCount`): disponibles tanto para jugadores como para espectadores mediante el botón `👀 Espectadores (+X)` (+1, -1, 0). Permite indicar personas adicionales que vienen únicamente a mirar o acompañar sin jugar; no consumen plazas de mesa.
  - La visualización de la actividad distingue claramente las secciones de `Jugadores` (`Assistents`) y `Espectadores`. En la lista de jugadores, cada entrada refleja de forma inequívoca si trae acompañantes a jugar (`+X a jugar`) o espectadores (`+Y a mirar`), por ejemplo: `Biel (+2 a jugar, +1 a mirar)`. En la lista de espectadores se indica `(+Y a mirar)` si vienen con espectadores adicionales.
  - Cambio de rol ágil: desde el detalle de la actividad, un jugador puede pasar a espectador (`👀 Pasar a espectador`), y un espectador puede pasar a jugador (`🎮 Pasar a jugar`, disponible sólo si quedan plazas libres en la mesa).
  - Al pulsar `Salir` (`Sortir`), la baja desapunta al usuario y libera tanto su plaza como todos los acompañantes y espectadores asociados.
...
- Publicación de snapshot a destinos de noticias suscritos; los feeds marcados por defecto como `events` llegan a todos los grupos de news habilitados salvo que ese feed tenga un destino explícito, incluido un topic. Al final del snapshot interno, el enlace localizado `Haz tu reserva` abre por privado el flujo `Crear actividad` mediante deep link. El feed separado `public-events` no se activa por defecto y publica sólo la agenda filtrada a actividades públicas, sin ese acceso reservado a socios. El bot recuerda el último snapshot por grupo/topic/categoría y borra el anterior tras publicar uno nuevo; si Telegram rechaza el borrado por antigüedad o permisos, edita el mensaje anterior a puntos suspensivos para que no queden dos calendarios largos visibles.
...
- El menú privado `Admin` ofrece `Catálogo web`: genera un enlace personal válido durante 1 hora y reutilizable durante toda su vigencia. Ese modo web vuelve a comprobar que el usuario siga siendo admin en cada petición, permite buscar y paginar el catálogo, muestra las carátulas disponibles y alterna entre lista y cuadrícula con 12, 24, 48 o 96 artículos por página, conservando esos ajustes durante la búsqueda, la paginación y el retorno tras editar. Permite editar los datos principales de cada juego y asignar o retirar el propietario mediante un selector de socios aprobados; `Guardar cambios` persiste la ficha y regresa al listado con confirmación. La ficha de edición muestra la carátula actual y permite traducir y guardar automáticamente sólo la descripción mediante una acción dedicada, sin necesitar después `Guardar cambios`. Familias y grupos no se muestran ni se modifican desde este editor. Desde cada editor puede cargar datos BGG cambiando libremente el texto de búsqueda, recorriendo la lista completa de coincidencias paginada de 20 en 20 y con su carátula, pegando una URL exacta y comparando cada edición física por carátula, idioma, editorial, año y código de producto antes de aplicar los datos; las consultas de detalles respetan el máximo de 20 IDs admitido por BGG. La ficha general se distingue explícitamente de las versiones físicas y, al guardar, la descripción se traduce automáticamente al castellano con el mismo servicio y fallback que usa el bot. Seleccionar una edición sólo sustituye sus metadatos específicos —idioma, editorial, año, código de producto y carátula—: el nombre visible y el original siempre proceden de la ficha principal del juego. Tanto la traducción manual como la aplicación de BGG muestran un bloqueo de espera accesible con indicador circular y la fase actual mientras dura la petición. Las escrituras quedan auditadas.
...
- Subida por DM: el usuario elige categoría con selector nivel a nivel y puede enviar adjuntos o pegar enlaces HTTPS públicos de archivos de Google Drive. En este último caso el bot descarga el binario con límite de tamaño, rechaza páginas privadas o no descargables, lo prepara como documento en Telegram y desde ahí lo incorpora al mismo lote y procesamiento que un adjunto manual. El flujo mantiene un recibo editable del total del lote, permite finalizar, revisar tags con opción de omitir y consultar una vista previa acotada para no superar el límite de Telegram con botones visibles para editar descripción, añadir tags, añadir imágenes o completar; acumula imágenes adicionales con recibo editable, avisa antes de completar sin tags y muestra progreso editable con estado por adjunto mientras copia al topic canónico, indexa y notifica suscripciones. Al terminar edita el recibo con enlaces directos a la entrada guardada y a su categoría.
