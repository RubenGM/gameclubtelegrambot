...
| Rol / partidas de rol                        | 🟢 Operativo        | Campañas/one-shots, personajes, sesiones, recurrencia anclada, invitaciones privadas y handouts internos.                             |
...
- Los one-shots piden fecha y hora en la creación, muestran una confirmación de Agenda con todos los datos exactos de la actividad y sólo después generan una primera sesión en `schedule_events` enlazada con `role_game_sessions`.
...
- La programación automática de campañas es un flag global persistido, apagado por defecto. Un admin puede cambiarlo desde el chat privado con `/role_auto_schedule enabled` o `/role_auto_schedule disabled`; mientras está apagado no se crean sesiones recurrentes ni al guardar una campaña/recurrencia ni desde el worker.
...
- La periodicidad de campañas ofrece `Sin días fijos`, `1` y `2` en el teclado y también acepta un número de semanas. `Sin días fijos` activa exclusivamente la programación manual; una frecuencia numérica pide el día de la semana mediante siete botones y después obliga a elegir la siguiente partida entre las cuatro próximas fechas coincidentes antes de pedir hora y ventana futura.
- Las campañas pueden reconfigurar esa periodicidad desde `Configurar recurrencia`. Con la programación automática activada, antes de guardar una planificación sin escrituras descuenta las sesiones futuras ya existentes, pregunta si se quieren escribir exactamente X actividades y enumera todos sus días y horas junto con los datos comunes completos. Al confirmar vuelve a calcular la planificación y, si algo ha cambiado, obliga a revisar de nuevo; sólo una confirmación vigente crea el lote mostrado. Con el flag apagado se guarda la regla sin crear actividades. La regla recurrente persiste la fecha elegida como anclaje `startsOn`, para que el worker mantenga la cadencia y la ventana futura sin inventar la primera ocurrencia; las sesiones canceladas enlazadas no se recrean.
...
| Rol / partidas de rol | `src/role-games/role-game-catalog.test.ts`, `src/role-games/role-game-catalog-store.test.ts`, `src/role-games/role-game-character-catalog.test.ts`, `src/role-games/role-game-character-store.test.ts`, `src/role-games/role-game-character-store.integration.test.ts`, `src/role-games/role-game-scheduler.test.ts`, `src/role-games/role-game-auto-scheduling-store.test.ts`, `src/telegram/role-game-participants.test.ts`, `src/telegram/role-game-flow.test.ts`, `src/telegram/role-game-character-flow.test.ts`, `src/bootstrap/create-app.test.ts`, `src/telegram/action-menu.test.ts`, `src/telegram/runtime-boundary.test.ts` |
